...
def valid_crop_resize(data_numpy, valid_frame_num, p_interval, window):
    ...
    if isinstance(data, np.ndarray):
        ...
        raise TypeError("Unsupported data type for data in valid_crop_resize")

    data = data.to(torch.float32)  # 确保数据类型为 float32

    # 调整数据形状以适应 interpolate 函数
    data = data.permute(0, 2, 3, 1).contiguous()  # C,V,M,T
    C, V, M, T = data.shape
    data = data.view(C * V * M, T)  # (C*V*M), T
    data = data.unsqueeze(0).unsqueeze(0)  # 1,1,(C*V*M),T

    # 使用 F.interpolate 进行插值
    data = F.interpolate(data, size=(C * V * M, window), mode='bilinear', align_corners=False)
    data = data.squeeze(0).squeeze(0)  # (C*V*M), window

    # 恢复数据形状
    data = data.view(C, V, M, window).permute(0, 3, 1, 2).contiguous()  # C,window,V,M

    # 修改部分结束

    return data  # 返回的是 torch.Tensor
...
def mean_subtractor(data_numpy, mean):
    ...
    if mean == 0:
        return data_numpy
    C, T, V, M = data_numpy.shape
    valid_frame = (data_numpy != 0).sum(axis=(0, 2, 3)) > 0
    begin = valid_frame.argmax()
    end = len(valid_frame) - valid_frame[::-1].argmax()
    data_numpy[:, begin:end, :, :] -= mean
    return data_numpy
